...
import java.util.ArrayList;
 ...
 public class Boy_Att extends Fragment {
     private RecyclerView arviewboyatt;
     ...
     String databasename = "BoyAttitude";
     String storagename = "BoyAttitude/";


     // private ArrayList<ImgLoad> imglistboyatt=new ArrayList<>();
     ...
     public View onCreateView(LayoutInflater inflater, ViewGroup container,
         ...
         // Toast.makeText(getContext(), "Boy Status", Toast.LENGTH_LONG).show();

         db = FirebaseFirestore.getInstance();
         sref = FirebaseStorage.getInstance().getReference();

         // Inflate the layout for this fragment
         View vv = inflater.inflate(R.layout.fragment_boy__att, container, false);
         arviewboyatt = vv.findViewById(R.id.boyattrecyclerview);
         pbar = vv.findViewById(R.id.progressBar5);
         ...
         arviewboyatt.setLayoutManager(new GridLayoutManager(getContext(), mNoOfColumns));
         //////////////////
         final ArrayList<ImgLoad> imglist = new ArrayList<>();
         ////////////
         final ProductAdapter productAdapter = new ProductAdapter(imglist);
         arviewboyatt.setAdapter(productAdapter);
         final CollectionReference productsRef = db.collection("BoyAttitude");
         ...
         query.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
             ...
             public void onComplete(@NonNull Task<QuerySnapshot> task) {
                 if (task.isSuccessful()) {
                     ...
                     arviewboyatt.addOnScrollListener(onScrollListener);
                 }
             }
         });
         ...
         return vv;
         ////////////////////
        /*dbboyatt.collection("BoyAttitude")
...
loadtv.setVisibility(View.GONE);
                    ...
                    Rvadpterboy adp = new Rvadpterboy(imglistboyatt);

                    //attach adapter
                    arviewboyatt.setAdapter(adp);
                    /////
                }//if close
                else {
     ...
     class  ProductAdapter extends RecyclerView.Adapter<ProductViewHolder>
     {
         ...
         public void onBindViewHolder(@NonNull final ProductViewHolder holder, final int position)
         {
             ...
             //get and load image
             sref.child("BoyAttitude/" + fimageuri).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                 @Override
...
//click listner clsoe
   ...
   /* class  Rvadpterboy extends RecyclerView.Adapter<Myviewholderboy>
    {
        private ArrayList<ImgLoad>imglistboyatt;

        public Rvadpterboy(ArrayList<ImgLoad> imglistboyatt) {
            this.imglistboyatt = imglistboyatt;
        }

        @NonNull
        @Override
        public Myviewholderboy onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
           View vvboy =getLayoutInflater().inflate(R.layout.recent_grid_design,parent,false);
            Myviewholderboy mvboy =new Myviewholderboy(vvboy);
            return mvboy;
        }

        @Override
        public void onBindViewHolder(@NonNull final Myviewholderboy holder, final int position) {

            final String fimageuri = imglistboyatt.get(position).getImgpath();
            //get and load image
            srefboyatt.child("BoyAttitude/" + fimageuri).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                @Override
                public void onSuccess(Uri uri) {
                    ...
                    try {
                        // Target iv= ((Target) holder).iv;
                        //Picasso.get().load(uri).into(iv);
                        Picasso.get().load(uri).placeholder(R.drawable.ic_rotate_24dp).into(holder.ivboyatt);
            ...
            //storege end
            //setonclick listner
            holder.all_lin_layboyatt.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                  ...
                  //  Toast.makeText(getContext(),"img path"+fimageuri,Toast.LENGTH_LONG).show();
                    ...
                    //Toast.makeText(getContext(),"img path"+fimageuri,Toast.LENGTH_LONG).show();
                    Intent intent=new Intent(getContext(),imgPview.class);
                    intent.putExtra("data",imglistboyatt);
                    intent.putExtra("position",position);
                    intent.putExtra("databasename",databasename);
                    intent.putExtra("storagename",storagename);
                    //intent.putExtra("post",position);
...
//click listner clsoe
        ...
        @Override
        public int getItemCount() {
            return imglistboyatt.size();
        }
    }
    //adpter close
    class Myviewholderboy extends RecyclerView.ViewHolder
    {
        public ImageView ivboyatt;
        public LinearLayout all_lin_layboyatt;

        public Myviewholderboy(@NonNull View itemView) {
            super(itemView);
            ivboyatt = itemView.findViewById(R.id.imageView2);
            //Toast.makeText(getContext(),"iv value in myviewholder class"+iv,Toast.LENGTH_LONG).show();
            all_lin_layboyatt = itemView.findViewById(R.id.recent_grid_lay);
